...
```{r echo=F, include=F}
#non-factors
TimeDfNF <- imputedSlimNF[,c(2:30)]
colnames(imputedSlimNF)
TimeDfRF <- within (TimeDfNF, {
  VoteTime <- factor(VoteTime, levels = 1:6, labels = c("Knew all along", "When candidate announced", "During convention", "Post-convention period", "Last two weeks of campaign", "On election day"))
})

glm.full.time <- glm(VoteTime ~ ., data = TimeDfNF)
glm.null.time <- glm(VoteTime ~ 1, data = TimeDfNF)

model.aic.forward.time <- step(glm.null.time, direction = "forward", trace = 0, scope = list(lower=glm.null.time, upper=glm.full.time))
```
###Vote Decision Time
####Ordered Logistic Regression
```{r cache=T}
summary(model.aic.forward.time)
```
