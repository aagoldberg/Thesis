...
dfHistPres <- dfCombSwitch[dfCombSwitch$Year %in% PresYears,]
dfHistMidTerm <- dfCombSwitch[dfCombSwitch$Year %in% MidTermYears,]
...
dfHistPresNum <- sapply(dfHistPres, as.numeric)

```

```{r barcharts}
HistCln <- data.frame(Reduce(cbind, Pres.cl.switchers))
colnames(HistCln) <- colnames(Pres.cl.switchers)

table(dfHistPres$ObsInfoLvlPost)
noninterval <- dfHistPresNum[,(!(colnames(dfHistPresNum) %in% intervalVars))]
head(dfHistPres)
codebook(dfHistPres$Gender)
colnames(noninterval)
